...
function App() {
  ...
  // Define your API URL
  const API_URL = "https://localhost:7245/api/Movies"; // Make sure to use http:// or https://
  ...
  // Added 'isLarge' prop to conditionally apply different sizes
  const MovieCard = ({ movie, isLarge = false }) => {
    // Conditional classes based on the 'isLarge' prop
    const outerDivClasses = `flex flex-col gap-3 ${isLarge ? 'pb-4' : 'pb-3'} ${isLarge ? 'min-w-[200px]' : 'w-full'}`;
    // For large cards, we'll try an aspect ratio of 2/3 (taller poster look) and a fixed width
      ...
      <div className="layout-container flex h-full grow flex-col">
        ...
        <div className="px-20 flex flex-1 justify-center py-5">
          <div className="layout-content-container flex flex-col w-full flex-1">
            ...
            {/* NEW: Highest Rated Movies Section */}
            ...
            <div className="flex overflow-y-auto [-ms-scrollbar-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <div className="flex items-stretch p-4 gap-7">
                {loading && <p className="text-white">Loading highest rated movies...</p>}
                {error && <p className="text-red-500">{error}</p>}
                {!loading && !error && movies.slice(0, 5).map(movie => (
            ...
            {/* Popular Movies Section - No changes here */}
